...
如果你是从其他渠道获得的那么意味着你的激活码实际上将会是无效的，那么软件将会有潜在的修改或者无法去掉的水印显示。
...
如果你是专业版本的或者有gitAccess的获取许可，只要拉下来最新的就可以了。
...
在3.5.2版本之后NGUI就包含了一个开源项目：C++预编译FreeType库
...
这个库只会在你选择"生成bitmap"字体选项的时候，并且它不会被包含在你的项目里。它只会在编辑器模式下被使用。
...
- 新增：Popup列表现在有了一个选项去控制它在按下或者点击的时候是否打开
- 新增：TweenAlpha现在也可以很好的控制light光源的渐变了
- 新增：更好的逻辑应用于UICamera.isOverUI, UICamera.uiHasFocus and UICamera.interactingWithUI.
...
- 新增：增加一个“保持值”选项在popup列表上用来使popup这个值存在即使在它消失之后。
...
- 新增：在规范的NGUI-Sprites和Label上现在可以指定custom风格的材质了
- 新增：新增在指定的panel上UV2支持。
- 新增：
