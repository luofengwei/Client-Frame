...
#region Convenience accessor properties
    ...
    static public UISprite AddButton(GameObject go)
    {
        UISprite w = AddSprite(go);
        w.name = "Button";
        w.autoResizeBoxCollider = true;

        w.gameObject.AddComponent<UIButton>();

        w.gameObject.AddComponent<UIEventListener>();

        BoxCollider boxCollider = (BoxCollider)w.gameObject.AddComponent<BoxCollider>();
        boxCollider.isTrigger = true;
        boxCollider.size = w.localSize;
